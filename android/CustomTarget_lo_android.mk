...
loandroid3_DIR := $(gb_CustomTarget_workdir)/android/app
...
$(loandroid3_DIR)/done : $(call gb_Postprocess_get_target,AllModulesButInstsetNative)
	...
	cd $(SRCDIR)/android/app && $(MAKE) all
...
# Us "foo" instead of the old INPATH
	...
	if test "$$ENABLE_ANDROID_LOK" != "TRUE" ; then cp $(SRCDIR)/android/app/build/outputs/apk/strippedUI$(if $(ENABLE_ANDROID_EDITING),Editing)/$(if $(ENABLE_RELEASE_BUILD),release/*-release-unsigned.apk,debug/*-debug.apk) $(BUILDDIR)/instsetoo_native/foo/bin ; fi
	$(call gb_Trace_EndRange,$(subst $(WORKDIR)/,,$@),MAK)
endif

$(call gb_CustomTarget_get_clean_target,android/loandroid3) :
	$(call gb_Output_announce,$(subst $(WORKDIR)/Clean/,,$@),$(false),MAK,2)
	cd $(SRCDIR)/android/app && $(MAKE) clean
